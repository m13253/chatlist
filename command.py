...
def trigger(xmpp, msg):
    try:
        ...
        if cmd[0]=='system':
            if from_jid in config.root:
                if len(cmd)>1:
                    msg.reply('\n'+subprocess.Popen(msg['body'].split(None, 1)[1], shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, close_fds=True).communicate()[0].decode('utf-8', 'replace')).send()
                else:
